...
// lixo/variantes comuns (PRIMARY, HOME, etc.)
...
// descritores/coleções que NÃO podem ficar no label (Shadow, Samurai, etc.)
...
// começos “multi-palavra” (para NÃO cortar Real Madrid, Manchester City, South Korea, etc.)
...
function cleanTeamValue(v?: string | null): string {
  ...
  // remove variações “tri-color/tricolour” mesmo se escaparem
...
/**
 * ✅ REGRA FINAL (garantia):
 * Se ainda vier "Chelsea Shadow" / "Japan Samurai" etc, corta para só o clube (1ª palavra),
 * exceto se for nome multi-palavra legítimo.
 */
function hardClampClubName(label: string): string {
  ...
  // regra “nunca apelido”: se não é multi-word starter, fica só 1ª palavra
  return parts[0];
}

/**
 * inferir pelo nome do produto (quando team vem vazio)
 */
...
/* ============================================================
   Filtro: Concept Kits (original concept designs)
============================================================ */
...
function isConceptKit(p: UIProduct): boolean {
  const n = normName(p);
  if (!n) return false;
        ...
        <div className="relative aspect-[3/4] sm:aspect-[4/5] bg-gradient-to-b from-slate-50 to-slate-100">
          ...
          <img
            ...
            onError={(e) => {
              ...
              if ((img as HTMLImageElement & { _fallbackApplied?: boolean })._fallbackApplied) return;
              (img as HTMLImageElement & { _fallbackApplied?: boolean })._fallbackApplied = true;
...
/* ============================================================
   ...
   - Traduzível com next-intl
   - Novo diretório com locale
============================================================ */

export default function ConceptKitsPage() {
  const t = useTranslations("conceptKitsPage");
  ...
  useEffect(() => {
    ...
    fetch(`/api/concept-kits`, { cache: "no-store" })
      ...
      .catch((e) => {
        if (!cancelled) {
          setResults([]);
          setError(e?.message || t("errors.fetchError"));
        }
      })
      .finally(() => {
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-500">
            ...
            {loading ? (
              ...
              <span>
                {t("found", { count: jerseysFiltered.length })}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            ...
            <div className="flex items-center gap-2 text-xs sm:text-sm">
              <span className="text-gray-500">{t("sortByLabel")}</span>
              <select
                value={sort}
                onChange={(e) => {
                  setSort(
