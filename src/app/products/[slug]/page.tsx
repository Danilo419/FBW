...
function buildUIProduct(args: {
  core: {
    ...
    description: string | null;
    basePrice: number; // cents
    imageUrls: string[] | null; // ← vem do DB
  };
  adultSizes: SizeUI[];
  kidsSizes: SizeUI[];
  optionGroups: OptionGroupUI[];
  ...
  const ui: ProductUI = {
    ...
    images: ensureArray(core.imageUrls), // ← mapeia imageUrls → images
    sizes: adultSizes,
    optionGroups,
  };
...
export default async function ProductPage({
  ...
  const core = await prisma.product.findUnique({
    ...
    select: {
      ...
      imageUrls: true,   // ✅ ler as imagens do DB
    },
  });
  if (!core) notFound();
  ...
  const uiProduct = buildUIProduct({
    core: {
      ...
      imageUrls: core.imageUrls, // ← passa para o builder
    },
    adultSizes: adult,
    kidsSizes: kids,
    optionGroups,
